...
\caption{Friendship network of a UK university faculty. Source: \textbf{igraphdata} R package (Csardi, 2015). Figure drawn using the R package \textbf{netplot} ( (https://github.com/usccana/netplot)}
\end{figure}\pause
...
1.  Make an initial guess of the model parameters using MPL (maximum pseudo likelihood), $\hat\theta_0$ \pause
...
2.  While the algorithm doesn't converge, do: \pause
    ...
    b.  Use the simulated sample of graphs to approximate the likelihood function. And with this approximation update the parameter $\theta_0$ using a Newton-Raphson step. \pause
...
- In addition, if a common data generating process can be assumed, a pooled version of the ERGMs can be estimated easily:\pause
...
Description of the \textcolor<2->{lightgray}{MCMC-}\textcolor<2->{USCCardinal}{MLE} algorithm [@Geyer1992]
...
By skiping the MCMC part we:\pause

1.  are able to get MLE estiamates directly,\pause

2.  *avoiding the degeneracy problem* latent in MCMC, and\pause
...
influence  <- readRDS("ci-and-ergms-models/ergmito_final_influence.rds")
leadership <- readRDS("ci-and-ergms-models/ergmito_final_leader.rds")
advice     <- readRDS("ci-and-ergms-models/ergmito_final_advice.rds")

# Subseting the models that Kayla presented in the Review
list(
  Advice     = advice$`RME+Female+SI3Fac1`,
  Influence  = influence$`T. Triad`,
  Leadership = leadership$Female
) %>% 
  texreg(
    caption = paste(
      "The two statistics that showed the most invariant were \\textbf{nodeicov.RME}",
      "and \\textbf{nodeocov.Female}. These two effects can be described as (1)",
...
- Permutation-like tests:\pause
    ...
    - Permutation: shuffle the dependent variable and compute, for example, correlations.\pause{} \textcolor{USCCardinal}{\textbf{This assumes the outcome is completely detached from the observation.}}\pause
...
\pause BTW Degree sequence $\mapsto$ Scale-free networks
...
- As a difference from permutation tests:\pause
    ...
    - On average, sampled networks will **look like the original** graph (addressing @broido2019's comments).


In principle, this would be equivalent to revise a rewiring test...
...
-   Ideally, we would like to build a test statistic with the following hypothesis\pause
...
-   Instead, we can take a look at the joint distribution of $(\Depvar, \Graph)$, and fix $\Depvar$ as follows:\pause
